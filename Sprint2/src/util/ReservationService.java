...
public class ReservationService {
    ...
    public static boolean validateBusinessHours(LocalDateTime dateTime) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                //  Convert using system default zone for Java 8+
                ZoneId systemZone = ZoneId.systemDefault();
                ZonedDateTime zoned = dateTime.atZone(systemZone);
                LocalTime normalized = zoned.toLocalTime();

                return !normalized.isBefore(openTime) && !normalized.isAfter(closeTime);
            } else {
                System.out.println(" No hours found for day " + dayOfWeek + ", using default 11:00–22:00");
                LocalTime fallbackOpen = LocalTime.of(11, 0);
                LocalTime fallbackClose = LocalTime.of(22, 0);
                return !localTime.isBefore(fallbackOpen) && !localTime.isAfter(fallbackClose);
            }
     ...
     */
    public static boolean hasConflict(int tableId, LocalDateTime newDateTime, int excludeId) {
        String sql =
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0;   // conflict if any record found within ±60 minutes
            }
        } catch (SQLException e) {
