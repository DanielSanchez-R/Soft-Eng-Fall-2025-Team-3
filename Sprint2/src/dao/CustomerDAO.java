...
public class CustomerDAO {
    ...
    public boolean deleteById(int id) {
        String getEmailSQL = "SELECT email FROM customers WHERE id=?";
        String deleteCustomerSQL = "DELETE FROM customers WHERE id=?";
        String deleteUserSQL = "DELETE FROM users WHERE email=? AND role='customer'";
        try (Connection conn = getConnection()) {
            // Get email first
            String email = null;
            try (PreparedStatement ps = conn.prepareStatement(getEmailSQL)) {
                ps.setInt(1, id);
                ResultSet rs = ps.executeQuery();
            ...
            // Delete from users mirror
            try (PreparedStatement ps = conn.prepareStatement(deleteUserSQL)) {
                ps.setString(1, email);
                ps.executeUpdate();
            }

            System.out.println(" Customer deleted (and mirrored user removed): " + email);
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
        }
